...
prepare_sciCSR <- function()
{
  ...
  if( ! 'scicsr' %in% conda_list()[, "name"]){
    conda_create(envname = "scicsr", python_version = "3.8")
    conda_install(envname = "scicsr", packages = c("scanpy==1.8.2", "deeptime==0.4.4", "scipy==1.10.0"), channel = "conda-forge")
    conda_install(envname = "scicsr", packages = "h5py", pip = TRUE, pip_options = "--force-reinstall")
    conda_install(envname = "scicsr", packages = "scvelo==0.2.5", pip = TRUE, pip_options = "-U")
    ...
    conda_install(envname = "scicsr", packages = "Numpy==1.23.5")
    conda_install(envname = "scicsr", packages = "matplotlib==3.6.3", pip = TRUE)
    conda_install(envname = "scicsr", packages = "pandas==1.5.3", pip = TRUE)
  }
